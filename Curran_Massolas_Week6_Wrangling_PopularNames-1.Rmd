...
1.) First, we want to find the most popular girl baby names in 1880, 1947, and 2013. Group the BabyNames dataframe by sex and by year.

2.) Filter out the boy names.

3.) Filter the data for the top ranked names for each year. 

4.) Filter for remaining names only from 1880, 1947, and 2013.

5.) Next, we want to create our glyph-ready table. Remove the "sex" variable. 

6.) Filter for only the names discovered in Step 4. Our table is ready to be graphed.
...
BabyNames %>% 
    ...
    filter(year %in% c(1880,1947,2013))
